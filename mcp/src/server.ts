...
export const REVIEW_PROMPT = {
  name: "review",
  description:
    "Generate a ranked forge-standard review prompt (critical->high->medium->low->nitpick) for the forge_review tool. Use with forge_review tool call.",
  arguments: [
    {
      name: "mode",
...
export function buildReviewPromptText(args: {
  ...
  const parts: string[] = [];
  parts.push(
    "You are a senior reviewer running forge-standard-review. Rank findings critical->high->medium->low->nitpick.",
  );
  ...
  parts.push(
    "Use the forge_review tool with the matching mode/range/file args and workdir. Keep artifacts in .forge-standard-review/ (report.md, review.json, learning.md). Local only, no network.",
  );
  return parts.join(" ");
}
...
export const FORGE_RESOURCES = [
  {
    ...
    description:
      "Ranked review report (critical->high->medium->low->nitpick) from .forge-standard-review/report.md or .forge-standard/report.md",
...
export function resolveReportPaths(repoRoot: string): string[] {
  return [
...
export function resolveReviewJsonPaths(repoRoot: string): string[] {
  return [
...
export function readFirstExisting(paths: string[]): { path: string; content: string } | null {
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
  ...
  const typed = (args ?? {}) as { mode?: string; file?: string; range?: string; focus?: string };
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  if (isReport) {
    ...
    if (!found) {
      throw new Error(
        `Report not found. Tried: ${resolveReportPaths(repoRoot).join(", ")} (repoRoot=${repoRoot}). Run forge_review first.`,
      );
    }
    return {
      contents: [
  ...
  if (isReviewJson) {
    ...
    if (!found) {
      throw new Error(
        `review.json not found. Tried: ${resolveReviewJsonPaths(repoRoot).join(", ")} (repoRoot=${repoRoot}). Run forge_review first.`,
      );
    }
    return {
      contents: [
